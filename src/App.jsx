...
export default function App() {
  ...
  const headerRef=useRef(null), sideRef=useRef(null), ganttRef=useRef(null), bodyRef=useRef(null), barRects=useRef({}), workloadOverviewRef=useRef(null), workloadGanttRef=useRef(null);
  ...
  const rowList = useMemo(()=>{
    ...
    if(view==="timeline"){const rows=[];teamMembers.forEach(m=>{const mt=[];filtered.forEach(p=>p.tasks.forEach(t=>{if(t.assignee===m.id)mt.push({...t,projName:p.name})}));if(mt.length>0){const sorted=sortByDependencyChain(mt);rows.push({type:"member",member:m,count:mt.length});sorted.forEach(t=>rows.push({type:"task",task:t,project:{name:t.projName}}))}});
      ...
      if(unassigned.length>0){const sorted=sortByDependencyChain(unassigned);rows.push({type:"member",member:{id:"unassigned",name:"未確定",role:"担当者未設定",color:"#9ca3af",av:"？",type:"internal"},count:unassigned.length});sorted.forEach(t=>rows.push({type:"task",task:t,project:{name:t.projName}}))}
      return rows}
  ...
  useEffect(()=>{if(!initialScrollRef.current&&ganttRef.current&&todayPos>0&&dbLoadedRef.current){initialScrollRef.current=true;setTimeout(()=>{if(ganttRef.current)ganttRef.current.scrollLeft=Math.max(0,todayPos)},150)}},[todayPos,loading]);
  useEffect(()=>{const h=e=>{
          ...
          <React.Fragment>
            <div style={ST.side}>
              ...
              {view==="gantt"&&showWorkloadOverview&&(
                <div ref={workloadOverviewRef} style={{maxHeight:220,overflowY:"auto",borderBottom:"2px solid #d1d5db",flexShrink:0}} onScroll={e=>{if(workloadGanttRef.current)workloadGanttRef.current.scrollTop=e.target.scrollTop}}>
                  {[...teamMembers,{id:"unassigned",name:"未確定",color:"#9ca3af",av:"？",hpw:40}].map(m=>{
                    const taskCount=projects.reduce((sum,p)=>sum+p.tasks.filter(t=>m.id==="unassigned"?!t.assignee:t.assignee===m.id).length,0);
                    return(<div key={"wm-"+m.id} style={{...ST.prow(true),gap:8,background:"#fafafa"}}><div style={ST.tav(m.color)}>{m.av}</div><div style={{flex:1}}>{m.name}</div><span style={{fontSize:11,color:"#6b7280"}}>{taskCount}</span></div>);
                  })}
                </div>
              )}
            ...
            <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden",background:"#f8f7f4"}}>
              ...
              {view==="gantt"&&showWorkloadOverview&&(
                <div ref={workloadGanttRef} style={{maxHeight:220,overflowY:"auto",overflowX:"auto",borderBottom:"2px solid #d1d5db",flexShrink:0}} onScroll={e=>{if(workloadOverviewRef.current)workloadOverviewRef.current.scrollTop=e.target.scrollTop;if(headerRef.current)headerRef.current.scrollLeft=e.target.scrollLeft;if(ganttRef.current)ganttRef.current.scrollLeft=e.target.scrollLeft}}>
                ...
                <div ref={bodyRef} style={{width:totalWidth,position:"relative",cursor:mActive?"crosshair":"default"}} onMouseDown={handleMStart} onDoubleClick={handleBodyDblClick}>
                  ...
                  {rowList.map(row=>{
                    ...
                    if(row.type==="member"){
                      ...
                      const mw=memberWorkloads[m.id]||{};
                      const maxUtil=Math.max(100,...Object.values(mw).map(w=>w.util||0));
                      return(<div key={"gr-"+m.id} style={{display:"flex",position:"sticky",top:0,height:44,borderBottom:"1px solid #e5e7eb",background:"#fafafa",alignItems:"flex-end",zIndex:10}}>
                        {Object.entries(mw).map(([key,w])=>{
                          const barHeight=maxUtil>0?Math.max(2,(w.util/maxUtil)*32):0;
                          const capColor=w.util>100?"#ef4444":w.util>80?"#f59e0b":m.color||"#10b981";
                          return(<div key={key} style={{position:"absolute",left:w.left,width:w.width,bottom:4,display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
      ...
      {ctxMenu&&(()=>{
        ...
        return<div style={{position:"fixed",left:adjustedX,top:adjustedY,background:"#fff",border:"1px solid #e5e7eb",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,.15)",zIndex:2000,minWidth:140,padding:4}}>
          ...
          {ctxMenu.type==="task"&&(()=>{const task=projects.flatMap(p=>p.tasks).find(t=>t.id===ctxMenu.id);const isMilestone=task?.type==="milestone";const targetIds=selIds.has(ctxMenu.id)&&selIds.size>1?Array.from(selIds):[ctxMenu.id];const isMulti=targetIds.length>1;return<React.Fragment>
            ...
            <div style={{position:"relative"}} onMouseEnter={e=>{const sub=e.currentTarget.querySelector('.submenu-assignee');const rect=e.currentTarget.getBoundingClientRect();const spaceBelow=window.innerHeight-rect.bottom;sub.style.display='block';if(spaceBelow<300){sub.style.top='auto';sub.style.bottom='0'}else{sub.style.top='0';sub.style.bottom='auto'}}} onMouseLeave={e=>e.currentTarget.querySelector('.submenu-assignee').style.display='none'}>
              ...
              <div className="submenu-assignee" style={{display:"none",position:"absolute",left:"100%",top:0,background:"#fff",border:"1px solid #e5e7eb",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,.15)",minWidth:160,padding:4,marginLeft:4,maxHeight:"min(300px, calc(100vh - 100px))",overflowY:"auto"}}>
                ...
                {teamMembers.some(m=>m.type==="external")&&<React.Fragment>
                  <div style={{height:1,background:"#e5e7eb",margin:"4px 0"}}/>
            ...
            <div style={{position:"relative"}} onMouseEnter={e=>{const sub=e.currentTarget.querySelector('.submenu');const rect=e.currentTarget.getBoundingClientRect();const spaceBelow=window.innerHeight-rect.bottom;sub.style.display='block';if(spaceBelow<200){sub.style.top='auto';sub.style.bottom='0'}else{sub.style.top='0';sub.style.bottom='auto'}}} onMouseLeave={e=>e.currentTarget.querySelector('.submenu').style.display='none'}>
              <button style={{width:"100%",padding:"8px 12px",border:"none",background:"transparent",textAlign:"left",cursor:"pointer",fontSize:12,borderRadius:4,display:"flex",alignItems:"center",gap:8,justifyContent:"space-between"}} onMouseEnter={e=>e.currentTarget.style.background="#f3f4f6"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>{"📂 プロジェクト移動"}<span style={{fontSize:11,color:"#9ca3af"}}>{"▶"}</span></button>
              <div className="submenu" style={{display:"none",position:"absolute",left:"100%",top:0,background:"#fff",border:"1px solid #e5e7eb",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,.15)",minWidth:160,padding:4,marginLeft:4,maxHeight:"min(250px, calc(100vh - 100px))",overflowY:"auto"}}>
